...
function RecursionNum(Data: number): number {
  if (Data === 1) {
    return 1
  }
...
/* 通过reduce计算阶乘 */
const RecuList = [5, 4, 3, 2, 1,]

function ReduceRecursion(list: number[]) {
...
function getFeiboNum(number: number): number {
...
得 32
*/
function findMax(list: number[]) {
  return list.reduce((pre, current) => {
    return pre > current ? pre : current
  })
}
const numberList = [1, 5, 10, 15, 13];
// console.log(findMax(numberList))

function findIndex(arr: number[]): number {
  if (arr.length === 0) {
    return undefined
  }
  if (arr.length === 1) {
  ...
  const leftMax = findIndex(arr.slice(0, mid));
  const rightMax = findIndex(arr.slice(mid));
